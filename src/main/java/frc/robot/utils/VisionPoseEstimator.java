...
//   /**
//    * Std devs scaler based on this specific camera, increase it if the resolution is lowered on
// this
...
//       /**
//        * The [xMeters, yMeters, thetaRadians] noise standard deviations of this pose estimate
// ([-1,
...
//                 })
//             .thenComparing(
//                 // this only happens when measurements land on the same timestamp, they need to
// be
...
//                       // if s1 (total) is greater than s2 (total), that actually means that we
// want
//                       // s2 to come
...
//   /**
//    * Returns an array of the new estimates since the last {@link #update} call. This should be
// used
...
//   //   var tagPose =
// FieldConstants.tagLayout.getTagPose(target.getFiducialId()).get().toPose2d();
...
//     if (isValid) {
//       double tagAmountStdDevsScaler = tagAmount == 1 ? VisionConstants.singleTagStdDevsScaler :
// 1;

//       double xStdDevs = Math.pow(avgTagDistance, 2) * tagAmountStdDevsScaler *
// cameraStdDevsScaler;
//       double yStdDevs = Math.pow(avgTagDistance, 2) * tagAmountStdDevsScaler *
// cameraStdDevsScaler;
//       double thetaStdDevs =
...
//       if (est.isPresent()) {
//         var newEstimate = processEstimate(est.get(),
// _gyroAtTime.apply(est.get().timestampSeconds));
